    ...
    def _setup_layout(self):
        ...
        self.app.layout = dbc.Container(
            [
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                html.P(
                                    "Real-time cost monitoring across AWS, Azure, and GCP",
                        ...
                        dbc.Col(
                            [
                                html.Div(
                                    id="last-update-time", className="text-right text-muted mb-2"
                                )
                            ],
                            width=6,
                        ),
                                ...
                                dbc.Card(
                                    [
                                        dbc.CardBody(
                                            [
                                                html.H4(
                                                    id="cost-trend-metric", className="card-title"
                                                ),
                                            ...
                                            [
                                                dcc.Loading(
                                                    [
                                                        dcc.Graph(
                                                            id="cost-trend-chart",
                                                            figure=self._create_initial_loading_chart(
                                                                "Daily Costs by Provider"
                                                            ),
                                                            config={
                ...
                dcc.Store(
                    id="date-range-type-store", data={"type": "latest"}
                ),  # Start with Latest mode by default
                dcc.Store(
                    id="show-all-accounts-store", data={"show_all": False}
                ),  # Track view all state
            ],
            fluid=True,
        )
        ...
        def update_data_store(
            ...
            try:
                ...
                if triggered_prop == "btn-latest":
                    ...
                    end_date_obj = today
                    logger.info(
                        f"🔘 BUTTON DEBUG: This Month calculated - start: {start_date_obj}, end: {end_date_obj}"
                    )
                elif triggered_prop == "btn-last-month":
                    start_date_obj, end_date_obj = self._get_last_month()
                    logger.info(
                        f"🔘 BUTTON DEBUG: Last Month calculated - start: {start_date_obj}, end: {end_date_obj}"
                    )
                    ...
                    if start_date_picker and end_date_picker:
                        ...
                        logger.info(
                            f"🔘 BUTTON DEBUG: Apply button - using picker dates: {start_date_obj} to {end_date_obj}"
                        )
                    else:
                        ...
                        end_date_obj = today
                        logger.warning(
                            "🔘 BUTTON DEBUG: Apply button - missing picker dates, using This Month fallback"
                        )
                    ...
                    end_date_obj = today
                    logger.info(
                        f"🔘 BUTTON DEBUG: Interval refresh using This Month - start: {start_date_obj}, end: {end_date_obj}"
                    )
                else:
                ...
                if not cached_data:
                    logger.info(
                        f"Cache miss - fetching data from API for {start_date_obj} to {end_date_obj}"
                    )
            ...
            try:
                ...
                if real_cost_data:
                    ...
                    for provider_name, provider_data in real_cost_data.provider_data.items():
                        ...
                        if isinstance(provider_data, dict):
                            service_breakdown[provider_name.lower()] = provider_data.get(
                                "service_breakdown", {}
                            )
                        else:
                            # Fallback for object attribute access
                            service_breakdown[provider_name.lower()] = getattr(
                                provider_data, "service_breakdown", {}
                            )
                    ...
                    for daily_entry in daily_costs:
                        if "date" in daily_entry:
                            ...
                            if "provider_breakdown" in daily_entry:
                                ...
                                logger.info(
                                    f"🔄 TRANSFORM DEBUG: {daily_entry['date']} - AWS: {aws_val}, GCP: {gcp_val}, Azure: {azure_val}"
                                )
                            else:
                                logger.warning(
                                    f"🔄 TRANSFORM DEBUG: No provider_breakdown for {daily_entry['date']}"
                                )
                    ...
                    logger.info(
                        f"🏦 API ACCOUNT DEBUG: account_breakdown from API type: {type(account_breakdown)}"
                    )
                    if account_breakdown and isinstance(account_breakdown, dict):
                        logger.info(
                            f"🏦 API ACCOUNT DEBUG: account_breakdown providers: {list(account_breakdown.keys())}"
                        )
                        for provider, accounts in account_breakdown.items():
                            if accounts:
                                ...
                                logger.info(
                                    f"🏦 API ACCOUNT DEBUG: {provider} sample from API: {sample_data}"
                                )
                                logger.info(
                                    f"🏦 API ACCOUNT DEBUG: {provider} sample keys from API: {list(sample_data.keys()) if isinstance(sample_data, dict) else 'not dict'}"
                                )
                    else:
                        logger.info(
                            f"🏦 API ACCOUNT DEBUG: account_breakdown is empty or not dict: {account_breakdown}"
                        )
                    ...
                    logger.info(
                        f"🏪 DATA STORE: Final cost_data structure: total={cost_data['total_cost']}, daily_count={len(cost_data['daily_costs'])}"
                    )
                    logger.info(
                        f"🏪 DATA STORE: Provider breakdown: {cost_data['provider_breakdown']}"
                    )
                    ...
                    if cost_data["daily_costs"]:
                        ...
                        azure_total = sum(
                            entry.get("azure", 0) for entry in cost_data["daily_costs"]
                        )
                        logger.info(
                            f"🏪 DATA STORE: Transformed daily totals - AWS: ${aws_total:.2f}, GCP: ${gcp_total:.2f}, Azure: ${azure_total:.2f}"
                        )

                        # Show last few entries with provider breakdown
                        for i, entry in enumerate(cost_data["daily_costs"][-3:]):
                            logger.info(
                                f"🏪 DATA STORE: Daily entry {len(cost_data['daily_costs'])-2+i}: {entry['date']} - AWS: ${entry.get('aws', 0):.2f}, GCP: ${entry.get('gcp', 0):.2f}"
                            )
                else:
                ...
                alert_data = {"active_alerts": 0, "critical_alerts": 0, "alerts": []}
                ...
                # Note: Date range calculation removed as it wasn't being used
                ...
                if data_fetch_time < 0.1:  # Cache hit (very fast response)
                    logger.info(
                        f"📊 CACHE HIT: Data loaded instantly ({data_fetch_time:.3f}s) - clearing loading state"
                    )
                    ...
                    if days_since_end > 2:
                        logger.warning(
                            f"📊 SAFETY: Historical data ({days_since_end} days old) showing 'in_progress' - forcing complete status"
                        )
                        loading_state = {"loading": False}
                    else:
                        logger.info(
                            "📊 API data collection status: IN PROGRESS - returning empty data to maintain loading state"
                        )
                    ...
                    logger.info("📊 API data collection status: COMPLETE - clearing loading state")
                    loading_state = {"loading": False}
                ...
                if "no partition of relation" in error_message and "found for row" in error_message:
                    ...
                    formatted_error = html.Div([f"❌ Error: {str(e)}"], className="error-message")
    ...
    def _setup_key_metrics_callback(self):
        ...
        def update_key_metrics(cost_data, selected_provider):
            ...
            if len(daily_values) >= 7:
                ...
                sorted_costs = sorted(
                    zip(daily_costs, daily_values, strict=False), key=lambda x: x[0]["date"]
                )
                last_7_days_values = [item[1] for item in sorted_costs[-7:]]
                prev_7_days_values = (
                    [item[1] for item in sorted_costs[-14:-7]] if len(sorted_costs) >= 14 else []
                )
                ...
                if prev_7_days_values and last_7_days_values:
                    ...
                    if prev_avg > 0:
                        ...
                        trend_class = (
                            ...
                            else "card-title text-danger"
                        )
                        trend_text = (
                            f"+{trend_percentage:.1f}%"
                            if trend_percentage > 0
                            else f"{trend_percentage:.1f}%"
                        )
    ...
    def _setup_loading_callbacks(self):
        ...
        def trigger_loading_state(*args):
            ...
            if ctx.triggered:
                ...
                logger.debug(
                    f"🔄 LOADING TRIGGERED by: {trigger_prop}"
        ...
        def update_loading_banner(loading_data, cost_data, start_date, end_date):
            ...
            should_show_loading = (
                loading_data
                and loading_data.get("loading", False)
                and (not cost_data or cost_data.get("data_collection_status") == "in_progress")
            )
        ...
        def update_cost_trend_chart(cost_data, selected_provider):
            ...
            logger.info(
                f"📊 CHART DEBUG: cost_data keys: {list(cost_data.keys()) if isinstance(cost_data, dict) else 'not dict'}"
            )
            ...
            if cost_data:
                if "daily_costs" in cost_data:
                    ...
                    if daily_costs:
                        ...
                        logger.info(
                            f"📊 CHART DEBUG: Sample entry keys: {list(sample_entry.keys())}"
                        )
                        ...
                        logger.info(
                            f"📊 CHART DEBUG: Provider totals - AWS: ${aws_total:.2f}, GCP: ${gcp_total:.2f}, Azure: ${azure_total:.2f}"
                        )
                else:
                    logger.warning("📊 CHART DEBUG: No 'daily_costs' in cost_data")

                if "provider_breakdown" in cost_data:
                    logger.info(
                        f"📊 CHART DEBUG: Provider breakdown: {cost_data['provider_breakdown']}"
                    )
            ...
            if cost_data and "daily_costs" in cost_data:
                ...
                logger.info(
                    f"📊 CHART DEBUG: Processing chart data with {len(daily_costs)} daily entries"
                )
                logger.info(
                    f"📊 CHART DEBUG: Date range: {dates[0] if dates else 'none'} to {dates[-1] if dates else 'none'}"
                )

                if selected_provider == "all":
                    logger.info("📊 CHART DEBUG: Creating grouped bars for all providers")
                    ...
                    logger.info(
                        f"📊 CHART DEBUG: Scale analysis - AWS max: ${aws_max:.2f}, GCP max: ${gcp_max:.2f}, Will use log: {will_use_log_scale}"
                    )
                    ...
                    for provider in ["aws", "azure", "gcp"]:
                        ...
                        if will_use_log_scale:
                            ...
                            hover_values = values
                            text_labels = [
                                self._format_currency_compact(v) if v > 0 else "$0.00"
                                for v in values
                            ]
                        else:
                        ...
                        logger.info(
                            f"📊 CHART DEBUG: {provider.upper()} - Total: ${total_value:.2f}, Non-zero entries: {non_zero_count}/{len(values)}"
                        )
                        logger.info(
                            f"📊 CHART DEBUG: {provider.upper()} values sample: {values[:3]}..."
                        )
                    ...
                    if selected_provider == "aws":
                        ...
                        for _i, (item, value) in enumerate(zip(daily_costs, values, strict=False)):
                            ...
                            if is_today:
                                ...
                                text_labels.append(
                                    self._format_currency_compact(value) if value > 0 else "$0.00"
                                )
                        ...
                        fig.add_trace(
                            go.Bar(
                                ...
                                marker_color=DashboardTheme.COLORS.get(
                                    selected_provider, "#000000"
                                ),
                                marker_line=dict(width=1, color="rgba(0,0,0,0.3)"),
                                text=text_labels,
                                textposition="outside",
                        ...
                        hover_values = values
                        text_labels = [
                            self._format_currency_compact(v) if v > 0 else "$0.00" for v in values
                        ]
            ...
            if cost_data and "daily_costs" in cost_data:
                ...
                if len(all_maxes) > 1:
                    ...
                    logger.info(
                        f"📊 CHART SCALE: Max ratio {ratio:.1f}x, using {'LOG' if use_log_scale else 'LINEAR'} scale"
                    )
                else:
                    use_log_scale = False
                    logger.info("📊 CHART SCALE: Single provider, using LINEAR scale")
            else:
                use_log_scale = False
            ...
            logger.info(
                f"📈 CHART FINAL: Generated chart with {len(fig.data)} traces, barmode={fig.layout.barmode}, title={fig.layout.title.text}"
            )
            ...
            for i, trace in enumerate(fig.data):
                ...
                logger.info(
                    f"📈 CHART TRACE {i}: {trace.name} - Total: ${trace_sum:.2f}, Data points: {len(trace.y) if hasattr(trace, 'y') else 0}"
                )

            logger.info(f"📈 CHART DEBUG: Chart generation completed in {chart_total_time:.3f}s")
            return fig

        @callback(Output("provider-breakdown-chart", "figure"), [Input("cost-data-store", "data")])
        def update_provider_breakdown_chart(cost_data):
            ...
            fig.update_layout(title="Provider Distribution", **DashboardTheme.LAYOUT)
        ...
        def update_service_breakdown_chart(cost_data, selected_provider):
            ...
            if (
                cost_data
                and "service_breakdown" in cost_data
                and selected_provider
                and selected_provider in cost_data["service_breakdown"]
            ):
        ...
        @callback(
            [Output("account-search-controls", "style"), Output("account-search-input", "style")],
        ...
        @callback(Output("account-summary", "children"), [Input("cost-data-store", "data")])
        def update_account_summary(cost_data):
            ...
            provider_summary = ", ".join(
                [f"{count} {provider.upper()}" for provider, count in provider_counts.items()]
            )
        ...
        def update_account_breakdown_content(
            cost_data, search_term, provider_filter, sort_option, show_all_data
        ):
            ...
            logger.info(
                f"🏦 ACCOUNT DEBUG: account_breakdown keys: {list(account_breakdown.keys())}"
            )
            for provider, accounts in account_breakdown.items():
                ...
                if accounts:
                    ...
                    logger.info(
                        f"🏦 ACCOUNT DEBUG: {provider} sample account keys: {list(sample_data.keys())}"
                    )
            ...
            for account in display_accounts:
                account["percentage"] = (
                    (account["total_cost"] / total_cost_all * 100) if total_cost_all > 0 else 0
                )
                ...
                table_rows.append(
                    html.Tr(
                        [
                            ...
                            html.Td([html.Div(account["account_name"], className="fw-bold")]),
                            html.Td(
                                dbc.Badge(
                                    account["provider"].upper(), color="info", className="me-2"
                                )
                            ),
                        ...
                        [
                            html.Tr(
                                [
                                    ...
                                    html.Th(
                                        "% of Total", className="text-end", style={"width": "20%"}
                                    ),
                                ]
                            )
                        ]
                        ...
                        [
                            dbc.Alert(
                                [
                                    ...
                                    html.Small(
                                        "(Tip: Use search/filters above to narrow results)",
                            ...
                            dbc.Alert(
                                [
                                    ...
                                    dbc.Button(
                                        ...
                                        id={"type": "view-all-btn", "index": len(accounts_list)},
                                        color="link",
                                        size="sm",
                                    ),
                                ],
        ...
        @callback(
            ...
            [Input("account-chart-btn", "n_clicks"), Input("cost-data-store", "data")],
            prevent_initial_call=False,
        )
        ...
        @callback(
            ...
            [Input("account-export-btn", "n_clicks"), Input("cost-data-store", "data")],
            prevent_initial_call=False,
        )
            ...
            if export_clicks and export_clicks > 0:
                ...
                if cost_data and "account_breakdown" in cost_data:
                    ...
                    for account in accounts_list:
                        account["percentage"] = (
                            (account["total_cost"] / total_cost_all * 100)
                            if total_cost_all > 0
                            else 0
                        )
        ...
        @callback(Output("cost-data-table", "children"), [Input("cost-data-store", "data")])
