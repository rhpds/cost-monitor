...
# Import providers to ensure they register themselves with the factory
try:
    from ..providers import aws, azure, gcp
except ImportError as e:
    logger.warning(f"Some providers could not be imported: {e}")
...
class CostDataManager:
    """Manages cost data retrieval and caching for the dashboard."""

    def __init__(self, config: CloudConfig):
        self.config = config
        self.auth_manager = MultiCloudAuthManager()
        self.normalizer = CostDataNormalizer()
        self._authenticated_providers = {}
        self._last_refresh = None

        # Cache for normalized/aggregated data
        self._normalization_cache = {}
        self._normalization_cache_ttl = 3600  # 1 hour TTL

    def _get_normalization_cache_key(self, start_date: date, end_date: date, providers: List[str]) -> str:
        """Generate cache key for normalized data."""
        import hashlib
        key_data = f"{start_date.isoformat()}:{end_date.isoformat()}:{':'.join(sorted(providers))}"
        return hashlib.md5(key_data.encode()).hexdigest()

    def _get_cached_normalized_data(self, cache_key: str) -> Optional[MultiCloudCostSummary]:
        """Get cached normalized data if valid."""
        if cache_key in self._normalization_cache:
            cached_data, timestamp = self._normalization_cache[cache_key]

            # Check if cache is still valid (TTL)
            import time
            if time.time() - timestamp < self._normalization_cache_ttl:
                logger.info(f"💾 Normalization cache HIT for {cache_key[:8]}...")
                return cached_data
            else:
                # Remove expired cache
                del self._normalization_cache[cache_key]
                logger.debug(f"💾 Normalization cache EXPIRED for {cache_key[:8]}...")

        logger.info(f"💾 Normalization cache MISS for {cache_key[:8]}...")
        return None

    def _cache_normalized_data(self, cache_key: str, data: MultiCloudCostSummary):
        """Cache normalized data."""
        import time
        self._normalization_cache[cache_key] = (data, time.time())
        logger.info(f"💾 Normalization cache STORED for {cache_key[:8]}...")

    async def initialize(self):
        """Initialize and authenticate cloud providers."""
        for provider_name in self.config.enabled_providers:
            ...
            try:
                ...
                if auth_result.success:
                    self._authenticated_providers[provider_name] = provider
                    logger.info(f"✅ Successfully authenticated {provider_name}")
                else:
    ...
    async def get_cost_data(
    ...
    ) -> Optional[MultiCloudCostSummary]:
        """Get aggregated cost data for all providers with normalization caching."""
        logger.info(f"Fetching cost data for {start_date} to {end_date} (force_refresh={force_refresh})")

        # Generate cache key for normalized data
        provider_names = list(self._authenticated_providers.keys())
        cache_key = self._get_normalization_cache_key(start_date, end_date, provider_names)

        # Check normalization cache first (unless force refresh)
        if not force_refresh:
            cached_result = self._get_cached_normalized_data(cache_key)
            if cached_result:
                logger.info(f"🚀 Returning cached normalized data - INSTANT response!")
                return cached_result

        # Create tasks for parallel provider data fetching
        async def fetch_provider_data(provider_name: str, provider):
            try:
                logger.debug(f"🔧 DEBUG: Calling {provider_name} provider...")
                cost_summary = await provider.get_cost_data(start_date, end_date)
                logger.debug(f"Retrieved cost data for {provider_name}: ${cost_summary.total_cost:.2f}")
                return cost_summary
            except Exception as e:
        ...
        tasks = [
            fetch_provider_data(provider_name, provider)
            for provider_name, provider in self._authenticated_providers.items()
        ]

        # Wait for all provider data fetches to complete
        ...
        if cost_summaries:
            # Set provider instances so normalizer can access cached data like account names
            self.normalizer.set_providers(self._authenticated_providers)

            # Start timing aggregation for performance monitoring
            start_time = time.time()
            aggregated_data = self.normalizer.aggregate_multi_cloud_data(cost_summaries)
            aggregation_time = time.time() - start_time

            # Cache the aggregated/normalized data for future requests
            self._cache_normalized_data(cache_key, aggregated_data)

            self._last_refresh = datetime.now()

            logger.info(f"Aggregated cost data for {start_date} to {end_date}, total: ${aggregated_data.total_cost:.2f}")
            logger.info(f"⚡ Performance: Data aggregation took {aggregation_time:.3f}s")

            if aggregation_time > 1.0:
                logger.warning(f"🐌 Slow aggregation detected: {aggregation_time:.3f}s")
    ...
    def clear_cache(self):
        """Clear all cached data at the provider level."""
        for provider_name, provider in self._authenticated_providers.items():
            # Clear provider-level cache if it has a clear_cache method
            if hasattr(provider, 'clear_cache'):
                try:
                    provider.clear_cache()
                    logger.info(f"Cleared {provider_name} provider cache")
                except Exception as e:
                    logger.warning(f"Failed to clear {provider_name} cache: {e}")

            # Also clear the individual cache files for AWS/GCP providers
            if hasattr(provider, '_cleanup_old_cache_files'):
                try:
                    provider._cleanup_old_cache_files()
                except Exception as e:
                    logger.warning(f"Failed to cleanup {provider_name} cache files: {e}")
    ...
    async def get_service_breakdown(
        ...
        """Get service cost breakdown for a specific provider."""
        if provider not in self._authenticated_providers:
            return {}

        try:
            return await self._authenticated_providers[provider].get_service_costs(
    ...
    async def get_account_breakdown_data(
        ...
        """Get cost data specifically for account breakdown with account grouping."""
        logger.info(f"Fetching account breakdown data for {start_date} to {end_date}")

        # Generate cache key for account breakdown normalized data
        provider_names = list(self._authenticated_providers.keys())
        cache_key = self._get_normalization_cache_key(start_date, end_date, provider_names + ["account_breakdown"])

        # Check normalization cache first (unless force refresh)
        if not force_refresh:
            cached_result = self._get_cached_normalized_data(cache_key)
            if cached_result:
                logger.info(f"🚀 Returning cached account breakdown data - INSTANT response!")
                return cached_result

        # Create tasks for parallel provider data fetching with account grouping
        async def fetch_account_data(provider_name: str, provider):
            try:
                # For AWS, use only LINKED_ACCOUNT grouping to avoid 5000 record limit
                # The dual grouping SERVICE + LINKED_ACCOUNT hits the limit due to alphabet sorting
                if provider_name == 'aws':
                    cost_summary = await provider.get_cost_data(
                        start_date, end_date, group_by=['LINKED_ACCOUNT']
                    )
                elif provider_name == 'gcp':
                    # For GCP, use PROJECT grouping to get individual project breakdown instead of MultiProject aggregation
                    cost_summary = await provider.get_cost_data(
                        start_date, end_date, group_by=['PROJECT']
                    )
                else:
                    # Other providers use their default grouping
                    cost_summary = await provider.get_cost_data(start_date, end_date)

                logger.debug(f"Retrieved account data for {provider_name}: ${cost_summary.total_cost:.2f}")
                return cost_summary
            except Exception as e:
                logger.error(f"Failed to get account data for {provider_name}: {e}")
                return None

        # Fetch data from all providers in parallel
        tasks = [
            fetch_account_data(provider_name, provider)
            for provider_name, provider in self._authenticated_providers.items()
        ]
        ...
        if cost_summaries:
            ...
            # Aggregate the account-specific data
            aggregated_data = self.normalizer.aggregate_multi_cloud_data(cost_summaries)

            # Cache the aggregated account breakdown data
            self._cache_normalized_data(cache_key, aggregated_data)

            logger.info(f"Account breakdown aggregated, total: ${aggregated_data.total_cost:.2f}")
            return aggregated_data
        return None

    async def _resolve_display_account_names(self, display_accounts: List[Dict[str, Any]]) -> None:
        """Resolve AWS account names for accounts that will be displayed in the UI.

        This optimizes performance by only resolving account names for accounts
        that will actually be shown to the user, avoiding expensive AWS Organizations
        API calls for accounts that remain hidden.

        Args:
            display_accounts: List of account dictionaries to resolve names for
        """
        if not display_accounts:
            return

        # Group accounts by provider for efficient resolution
        aws_accounts = []
        for account in display_accounts:
            if account.get('provider') == 'aws':
                # Only resolve if the account name is still the raw account ID (not yet resolved)
                account_id = account.get('account_id')
                account_name = account.get('account_name', '')
                # Check if the account name is just the account ID (needs resolution)
                if account_id and (account_name == account_id or not account_name):
                    aws_accounts.append(account_id)

        if not aws_accounts:
            return  # No AWS accounts need resolution

        # Get AWS provider instance
        aws_provider = self._authenticated_providers.get('aws')
        if not aws_provider:
            logger.debug("No authenticated AWS provider available for account name resolution")
            return

        try:
            # Resolve account names for AWS accounts
            logger.info(f"🔵 Dashboard: Resolving names for {len(aws_accounts)} displayed AWS accounts: {aws_accounts}")
            logger.info(f"🔵 Dashboard: Current cache has {len(aws_provider.account_names_cache)} entries")
            account_name_mapping = await aws_provider.resolve_account_names_for_ids(aws_accounts)

            # Update the display_accounts with resolved names
            for account in display_accounts:
                if account.get('provider') == 'aws':
                    account_id = account.get('account_id')
                    if account_id in account_name_mapping:
                        resolved_name = account_name_mapping[account_id]
                        formatted_name = aws_provider.format_account_display_name(account_id, resolved_name)
                        account['account_name'] = formatted_name
                        logger.debug(f"🔵 Dashboard: Updated {account_id} -> {formatted_name}")

            logger.info(f"🔵 Dashboard: Account name resolution completed for displayed accounts")

        except Exception as e:
            logger.warning(f"Failed to resolve account names for displayed accounts: {e}")
            # Continue without account name resolution - accounts will show with IDs
...
class CostMonitorDashboard:
    ...
    def _setup_callbacks(self):
        ...
        logger.info("✅ All dashboard callbacks have been set up successfully")
